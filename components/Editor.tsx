...
const Editor = ({entry}) =>{
    ...
    useAutosave({
        data:value,
        // _value the value that changed, we can just grab value but it might be a few cycles behind in the render cycle so may not be up to date
        onSave: async(_value) =>{
            setIsLoading(true)
...
return (
    <div className="w-full h-full grid grid-cols-3">
        ...
        <textarea className="w-full h-full p-8 text-xl outline-none" value={value} onChange={(e)=>setValue(e.target.value)}/>
          ...
          <div className="bg-blue-300 px-6 py-10" style={{backgroundColor:color}}>
